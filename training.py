...
class Training():
    def __init__(self, model, accelerator, num = 0):
        super().__init__()
        # load tpu info
        with open('data/accelerator.json') as a_file:
            accelerators = json.load(a_file)
        self.model = model
        self.accelerator = accelerators[accelerator]
        self.num = num
        return
    
    def calc_train_time(self,):
        self.training_seconds = self.model.total_tflops / (self.tflop_per_sec * self.num)
    ...
    def calc_tpu_tflops(self):
        ...
        self.tflop_per_sec = self.accelerator['peak TFLOPS'] * self.accelerator['hardware_utilization']

    # calculate tflops using imperical hardware utilization
    def calc_tflops(self, hardware_efficiency):
        self.tflop_per_sec = self.accelerator['peak TFLOPS'] * hardware_efficiency
    ...
    def predict_num_throughput(self, node_size):
        ...
        gpu_cap = 0.03 * self.accelerator['memory']
        ...
        if self.accelerator['memory'] != 80:
            num_gpu = np.round(num_gpu * 2.5)
        if self.accelerator['memory'] == 40:
            num_gpu *= 2
        ...
        if self.accelerator['memory'] != 80:
            X_new = X -  X*rel_thru
            peak_new = X_new /312
            self.tflop_per_sec= peak_new*125
        else:
            self.tflop_per_sec = X
        self.num = num_gpu
